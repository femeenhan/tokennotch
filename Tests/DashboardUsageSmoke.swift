...
@main @MainActor enum DashboardUsageSmoke {
    static func main() throws {
        ...
        precondition(TokenCostEstimate(records: model.dashboard.usage).isPartial)
        for scheme in [ColorScheme.light, .dark] {
            let host = NSHostingView(rootView: DashboardView(model: model).preferredColorScheme(scheme))
            let window = NSWindow(contentRect: CGRect(x: 0, y: 0, width: 960, height: 740), styleMask: [], backing: .buffered, defer: false)
            window.contentView = host
            window.display(); host.layoutSubtreeIfNeeded()
            let image = host.bitmapImageRepForCachingDisplay(in: host.bounds)!
            host.cacheDisplay(in: host.bounds, to: image)
            try image.representation(using: .png, properties: [:])!.write(to: URL(fileURLWithPath: "/tmp/build-spirit-dashboard-\(scheme).png"))
            precondition(host.bounds.width == 960 && host.bounds.height == 740)
            window.contentView = nil
        }
        print("PASS: dashboard renders populated Astra/Spark token and partial API cost states in both appearances")
    }
}
